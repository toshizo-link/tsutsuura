...
use Tsutsuura\Server\Auth\LogEmailSender;
use Tsutsuura\Server\Auth\DisabledEmailSender;
...
try {
    ...
    $emailSender = match ($config->emailDriver) {
        'mail' => new NativeEmailSender($config),
        'log' => new LogEmailSender($config),
        default => new DisabledEmailSender(),
    };
    $emailOtp = new EmailOtpService($database, $crypto, $rateLimiter, $emailSender, $users, $sessions, $config);
    ...
    $health = static function () use ($database, $config, $answerMedia): never {
        ...
        try {
            ...
            foreach ([
                ...
                '008_question_schedule.sql',
                '009_profile_marks.sql',
                '010_email_auth.sql',
                '011_content_safety.sql',
            ] as $version) {
            ...
            $pdo->query('SELECT email_address, email_verified_at FROM users WHERE 1 = 0');
            $pdo->query('SELECT 1 FROM email_otp_challenges WHERE 1 = 0');
            $pdo->query('SELECT 1 FROM email_enrollment_challenges WHERE 1 = 0');
            $pdo->query(
            ...
            $pdo->query('SELECT 1 FROM answer_reports WHERE 1 = 0');
            $pdo->query('SELECT 1 FROM user_blocks WHERE 1 = 0');
            $pdo->query('SELECT 1 FROM moderation_actions WHERE 1 = 0');
            ...
            Response::json(['data' => [
                ...
                'authProviders' => [
                    ...
                    'email' => $config->emailDriver !== 'disabled',
                ],
                'capabilities' => [
        ...
        Response::json(['data' => [
            'status' => 'ok',
            'apiRevision' => '2026-09-07-safety',
            'schemaVersion' => '011_content_safety.sql',
            'database' => 'ok',
            'authProviders' => [
                'phone' => $config->otpDriver !== 'disabled',
                'email' => $config->emailDriver !== 'disabled',
            ...
            'capabilities' => [
                ...
                'userBlocking' => true,
                'answerReporting' => true,
                'commentReporting' => true,
                'reportedContentHiding' => true,
                'operatorModeration' => true,
            ],
            'time' => gmdate(DATE_ATOM),
        ]]);
    };
    ...
    $router->add('POST', '/v1/auth/email/request', static function (Request $request) use ($emailOtp, $config): never {
        $body = $request->json();
        $result = $emailOtp->request($body['email'] ?? null, ClientIp::resolve($request, $config));
        Response::json(['data' => $result], 202);
    });
    $router->add('POST', '/v1/auth/email/verify', static function (Request $request) use ($emailOtp, $config): never {
        $body = $request->json();
        $result = $emailOtp->verify(
            $body['requestId'] ?? null,
            $body['code'] ?? null,
            ClientIp::resolve($request, $config),
            $request->header('idempotency-key'),
    ...
    $router->add('GET', '/v1/me', static function (Request $request) use ($sessions): never {
        Response::json(['data' => Presenter::user($sessions->requireUser($request), true)]);
    });
    $router->add('PATCH', '/v1/me', static function (Request $request) use ($sessions, $users, $app): never {
        $user = $sessions->requireUser($request);
        $body = $request->json();
        $profile = Presenter::user($users->updateProfile(
            (int) $user['id'],
            $body['displayName'] ?? null,
            $body['avatarMark'] ?? null,
            array_key_exists('avatarMark', $body),
        ), true);
        // Name edits can also update a generated family name. Return that
        // authoritative family immediately so Settings does not keep stale text.
        $profile['family'] = $app->family((int) $user['id']);
    ...
    $router->add('POST', '/v1/me/email/request', static function (
        Request $request,
    ) use ($sessions, $emailOtp, $config): never {
        $user = $sessions->requireUser($request);
        $body = $request->json();
        Response::json(['data' => $emailOtp->requestEnrollment(
            (int) $user['id'],
            $body['email'] ?? null,
            ClientIp::resolve($request, $config),
        )], 202);
    });
    $router->add('POST', '/v1/me/email/verify', static function (
        Request $request,
    ) use ($sessions, $emailOtp, $config): never {
        $user = $sessions->requireUser($request);
        $body = $request->json();
        Response::json(['data' => ['user' => $emailOtp->verifyEnrollment(
            (int) $user['id'],
            $body['requestId'] ?? null,
            $body['code'] ?? null,
            ClientIp::resolve($request, $config),
    ...
    $router->add('GET', '/v1/me/blocks', static function (Request $request) use ($sessions, $database): never {
        $user = $sessions->requireUser($request);
        Response::json(['data' => (new ContentSafety($database))->snapshot((int) $user['id'])]);
    });
    $router->add('PUT', '/v1/me/blocks/{userId}', static function (Request $request, array $parameters) use ($sessions, $database): never {
        $user = $sessions->requireUser($request);
        (new ContentSafety($database))->block((int) $user['id'], $parameters['userId']);
        Response::json(['data' => (object) []]);
    });
    $router->add('DELETE', '/v1/me/blocks/{userId}', static function (Request $request, array $parameters) use ($sessions, $database): never {
        $user = $sessions->requireUser($request);
        (new ContentSafety($database))->unblock((int) $user['id'], $parameters['userId']);
        Response::json(['data' => (object) []]);
    });
    $router->add('POST', '/v1/answers/{answerId}/reports', static function (Request $request, array $parameters) use ($sessions, $app): never {
        $user = $sessions->requireUser($request);
        $body = $request->json();
        Response::json(['data' => ['report' => $app->reportAnswer(
            (int) $user['id'], $parameters['answerId'], $body['reason'] ?? null, $body['details'] ?? null,
        )]], 201);
    });
    ...
    $router->add('PATCH', '/v1/me/timezone', static function (Request $request) use ($sessions, $lifecycle): never {
        $user = $sessions->requireUser($request);
        Response::json(['data' => $lifecycle->updateDeviceTimezone((int) $user['id'], $request->json())]);
    });
    ...
    $submitAnswer = static function (
        ...
        if ($request->isMultipart()) {
            ...
            $unexpectedFields = array_diff(
                array_keys($body),
                ['body', 'audioDurationMilliseconds', 'questionId', 'questionDate'],
            );
            ...
            $answer = $app->submitTodayAnswer(
                (int) $user['id'],
                $body['body'] ?? null,
                expectedQuestionId: $body['questionId'] ?? null,
                expectedQuestionDate: $body['questionDate'] ?? null,
            );
        }
...
function applyCors(Request $request, Config $config): void
{
    ...
    header('Access-Control-Allow-Headers: Authorization, Content-Type, Range, X-Request-ID, Idempotency-Key, X-HTTP-Method-Override');
