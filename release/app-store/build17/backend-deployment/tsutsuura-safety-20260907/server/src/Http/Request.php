...
final class Request
{
    ...
    public static function fromGlobals(): self
    {
        ...
        // Some shared hosts reject PATCH/PUT/DELETE before PHP is reached.
        // A POST transport may select the same existing mutation handler;
        // authentication, validation and idempotency still belong to that route.
        if (array_key_exists('x-http-method-override', $headers)) {
            ...
            if ($method !== 'POST' || !in_array($override, ['PATCH', 'PUT', 'DELETE'], true)) {
                throw new ApiException(
                    ...
                    'X-HTTP-Method-Override requires POST and must be PATCH, PUT or DELETE.',
                );
            }
            $method = $override;
        }
