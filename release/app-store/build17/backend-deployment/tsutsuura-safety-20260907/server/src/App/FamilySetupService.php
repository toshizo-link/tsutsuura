...
final class FamilySetupService
{
    ...
    public function createOrganizerFamily(
        ...
        return $this->database->transaction(function (PDO $pdo) use (
            ...
            $familyId = $this->insertFamily($pdo, $familyName, $familyName === UserService::defaultFamilyName($organizerName));
            $member = $pdo->prepare(
    ...
    public function pairings(int $actorUserId): array
    {
        ...
        $sql =
            'SELECT dp.id, dp.family_id, dp.managed_user_id, dp.created_by_user_id,
                    ...
                    u.display_name AS member_name, u.avatar_mark AS member_avatar_mark, f.name AS family_name
    ...
    private function presentPairing(
        ...
        $pairing = [
            ...
            'member' => [
                ...
                'avatarMark' => $row['member_avatar_mark'],
            ],
            'family' => [
    ...
    private function insertFamily(PDO $pdo, string $familyName, bool $tracksOwner): int
    {
        for ($attempt = 0; $attempt < 5; $attempt++) {
            $inviteCode = strtoupper(substr(bin2hex(random_bytes(8)), 0, 12));
            try {
                $family = $pdo->prepare(
                    'INSERT INTO families (name, invite_code, name_tracks_owner) VALUES (:name, :code, :tracks)'
                );
                $family->execute(['name' => $familyName, 'code' => $inviteCode, 'tracks' => (int) $tracksOwner]);
