...
final class AnswerMediaService
{
    ...
    /**
     * Recognizes a retry without replacing published attachments.
     * @param list<array<string, mixed>> $prepared
     */
    public function matchesPreparedForAnswer(PDO $pdo, int $answerId, array $prepared): bool
    {
        $statement = $pdo->prepare(
            "SELECT kind, sort_order, storage_key, file_name, mime_type, byte_count, duration_ms
             FROM answer_media WHERE answer_id = :answer
             ORDER BY CASE kind WHEN 'audio' THEN 0 ELSE 1 END, sort_order, id"
        );
        $statement->execute(['answer' => $answerId]);
        $stored = $statement->fetchAll();
        if (count($stored) !== count($prepared)) {
            return false;
        }
        foreach ($stored as $index => $row) {
            $candidate = $prepared[$index];
            if ((string) $row['kind'] !== $candidate['kind'] ||
                (int) $row['sort_order'] !== $candidate['position'] ||
                $row['file_name'] !== $candidate['fileName'] ||
                (string) $row['mime_type'] !== $candidate['mimeType'] ||
                (int) $row['byte_count'] !== $candidate['byteCount'] ||
                ($row['duration_ms'] === null ? null : (int) $row['duration_ms']) !== $candidate['durationMilliseconds']) {
                return false;
            }
            $storedPath = $this->pathForStorageKey((string) $row['storage_key']);
            $candidatePath = $this->pathForStorageKey($candidate['storageKey']);
            if (!is_file($storedPath) || !is_file($candidatePath) ||
                hash_file('sha256', $storedPath) !== hash_file('sha256', $candidatePath)) {
                return false;
            }
        }
        return true;
    ...
    public function download(int $userId, string $mediaIdValue, string $token): array
    {
        ...
        $statement = $this->database->connection()->prepare(
            'SELECT am.id, am.storage_key, am.file_name, am.mime_type, am.byte_count
             ...
             WHERE am.id = :media AND fm.user_id = :user AND ' . ContentSafety::answerSQL('a', $userId) . '
             LIMIT 1'
        );
