...
final class AppService
{
    ...
    public function home(int $userId, mixed $cursorValue = null): array
    {
        ...
        $answerHidden = $myAnswerId !== false && !ContentSafety::canSeeAnswer($this->database->connection(), $userId, (int) $myAnswerId);
        return [
            'family' => $family,
            'todayQuestion' => $this->question($question, $today, $myAnswerId !== false, $answerHidden),
            'todayAnsweredUserIDs' => $todayAnsweredUserIds,
            'myAnswer' => $myAnswerId === false || $answerHidden ? null : $this->answer((int) $myAnswerId, $userId),
            'feed' => $page['items'],
    ...
    public function todayQuestion(int $userId): array
    {
        ...
        $answerHidden = $answerId !== false && !ContentSafety::canSeeAnswer($this->database->connection(), $userId, (int) $answerId);
        return [
            'question' => $this->question($question, $today, $answerId !== false, $answerHidden),
            'answer' => $answerId === false || $answerHidden ? null : $this->answer((int) $answerId, $userId),
        ];
    }
    ...
    public function setLike(int $userId, string $answerIdValue, bool $liked): array
    {
        ...
        $result = $this->database->transaction(function (PDO $pdo) use (
            ...
            $count = $pdo->prepare(
                'SELECT COUNT(*) FROM answer_likes al WHERE answer_id = :answer AND ' . ContentSafety::contactSQL('al.user_id', $userId)
            );
    ...
    public function comments(int $userId, string $answerIdValue, mixed $cursorValue, mixed $limitValue): array
    {
        ...
        $statement = $this->database->connection()->prepare(
            'SELECT c.id, c.answer_id, c.user_id, c.parent_comment_id,
             ...
             WHERE c.answer_id = :answer AND c.id > :cursor AND ' . ContentSafety::commentSQL('c', $userId) . '
             ORDER BY c.id ASC
             LIMIT :limit'
        );
    ...
    ): array
    {
        ...
        $comment = $this->database->transaction(function (PDO $pdo) use (
            ...
            $this->lockedFamilyRow($pdo, $userId);
            $this->assertAnswerVisible($answerId, $userId);
            if ($parentCommentId !== null && !ContentSafety::canSeeComment($pdo, $userId, $parentCommentId)) {
            ...
            if ($requestKeyHash !== null && $requestFingerprint !== null) {
                ...
                if ($replay !== null) {
                    ...
                    $exists->execute(['id' => $replay['id']]);
                    // Preserve the existing receipt contract after a user's
                    // own deletion, but never replay moderator-hidden content.
                    if ($exists->fetchColumn() !== false && !ContentSafety::canSeeComment($pdo, $userId, (int) $replay['id'])) {
    ...
    public function updateComment(int $userId, string $commentIdValue, mixed $bodyValue): array
    {
        ...
        $body = self::body($bodyValue, 1000, 'comment');
        return $this->database->transaction(function (PDO $pdo) use ($userId, $commentId, $body): array {
            $this->lockedFamilyRow($pdo, $userId);
            if (!ContentSafety::canSeeComment($pdo, $userId, $commentId)) {
                throw new ApiException(404, 'comment_not_found', 'Comment not found.');
            }
            $statement = $pdo->prepare(
    ...
    public function reportComment(
        ...
        return (new ContentSafety($this->database))->report($userId, 'comment', $commentIdValue, $reasonValue, $detailsValue);
    }

    public function reportAnswer(int $userId, string $answerIdValue, mixed $reasonValue, mixed $detailsValue = null): array
    {
        return (new ContentSafety($this->database))->report($userId, 'answer', $answerIdValue, $reasonValue, $detailsValue);
    }
    ...
    private function answerRows(
        ...
        $sql =
            'SELECT a.id, a.user_id, a.answer_date, a.body, a.created_at, a.updated_at,
                    ...
                    u.phone_e164, u.display_name, u.avatar_url, u.avatar_mark, u.created_at AS user_created_at,
                    (SELECT COUNT(*) FROM answer_likes al WHERE al.answer_id = a.id AND ' . ContentSafety::contactSQL('al.user_id', $viewerId) . ') AS likes_count,
                    EXISTS(SELECT 1 FROM answer_likes al2 WHERE al2.answer_id = a.id AND al2.user_id = :viewer) AS is_liked,
                    (SELECT COUNT(*) FROM comments c WHERE c.answer_id = a.id AND ' . ContentSafety::commentSQL('c', $viewerId) . ') AS comments_count
             FROM answers a
             JOIN questions q ON q.id = a.question_id
             JOIN users u ON u.id = a.user_id
             WHERE a.family_id = :family AND ' . ContentSafety::answerSQL('a', $viewerId);
        if ($beforeId !== null) {
    ...
    private function answer(int $answerId, int $viewerId): array
    {
        ...
        $statement = $this->database->connection()->prepare(
            'SELECT a.id, a.user_id, a.answer_date, a.body, a.created_at, a.updated_at,
             ...
             JOIN questions q ON q.id = a.question_id
             JOIN users u ON u.id = a.user_id
             WHERE a.id = :answer AND a.family_id = :family AND ' . ContentSafety::answerSQL('a', $viewerId)
        );
    ...
    private function assertAnswerVisible(int $answerId, int $userId): void
    {
        $statement = $this->database->connection()->prepare(
            'SELECT 1 FROM answers a
             JOIN family_members fm ON fm.family_id = a.family_id
             WHERE a.id = :answer AND fm.user_id = :user AND ' . ContentSafety::answerSQL('a', $userId)
        );
        $statement->execute(['answer' => $answerId, 'user' => $userId]);
     ...
     *  @return array<string, mixed>
     */
    private function question(array $row, string $date, bool $hasAnswered, bool $answerHidden): array
    {
        return [
            'id' => (string) $row['id'],
