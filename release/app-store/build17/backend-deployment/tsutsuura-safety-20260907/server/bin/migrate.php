...
foreach ($files as $file) {
    ...
    // Question metadata is additive. Resume either ALTER independently after
    // interruption; CREATE TABLE/INDEX statements are already idempotent.
    if ($version === '008_question_schedule.sql') {
        foreach (['publish_start_minute', 'publish_end_minute'] as $column) {
            if (hasColumn($pdo, $config->dbConnection, 'questions', $column)) {
                $sql = preg_replace(
                    '/ALTER TABLE questions ADD COLUMN ' . $column . '\\b.*?;\\s*/s',
                    '',
                    $sql,
                    1,
                ) ?? $sql;
    ...
    // Every ALTER and index in the email migration can resume independently;
    // CREATE TABLE IF NOT EXISTS is safe after interrupted MySQL DDL commits.
    if ($version === '010_email_auth.sql') {
        foreach (['email_address', 'email_verified_at'] as $column) {
            if (hasColumn($pdo, $config->dbConnection, 'users', $column)) {
                $sql = preg_replace('/ALTER TABLE users ADD COLUMN ' . $column . '\\b.*?;\\s*/s', '', $sql, 1) ?? $sql;
            }
        }
        foreach ([['users', 'users_email_unique'], ['email_otp_challenges', 'email_otp_expiry_index'],
            ['email_enrollment_challenges', 'email_enrollment_user_index'],
            ['email_enrollment_challenges', 'email_enrollment_expiry_index']] as [$table, $index]) {
            if (hasIndex($pdo, $config->dbConnection, $table, $index)) {
                $sql = preg_replace('/CREATE (?:UNIQUE )?INDEX (?:IF NOT EXISTS )?' . $index . '\\b.*?;\\s*/s', '', $sql, 1) ?? $sql;
            }
        }
    }
    ...
    if ($version === '009_profile_marks.sql') {
        addColumnIfMissing($pdo, $config->dbConnection, 'users', 'avatar_mark',
            $config->dbConnection === 'sqlite'
                ? "TEXT NULL CHECK (avatar_mark IS NULL OR (length(avatar_mark) = 256 AND avatar_mark NOT GLOB '*[^01]*'))"
                : 'VARCHAR(256) CHARACTER SET ascii COLLATE ascii_bin NULL');
        addColumnIfMissing($pdo, $config->dbConnection, 'families', 'name_tracks_owner',
            $config->dbConnection === 'sqlite'
                ? 'INTEGER NOT NULL DEFAULT 0 CHECK (name_tracks_owner IN (0, 1))'
                : 'TINYINT(1) NOT NULL DEFAULT 0');
        $backfill = preg_replace('/ALTER TABLE\s+[^;]+;\s*/s', '', $sql) ?? $sql;
        $pdo->exec($backfill);
    } elseif ($version === '007_idempotent_mutations.sql') {
        applyIdempotentMutationMigration($pdo, $config->dbConnection);
    } else {
...
if (in_array('--seed', $argv, true)) {
    ...
    foreach ($seedFiles as $file) {
        ...
        // Seeds insert only missing prompts, allowing the yearly catalog to
        // reach existing installations without replacing question/answer IDs.
        $pdo->exec($sql);
