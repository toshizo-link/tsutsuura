#!/usr/bin/env python3
"""Read-only audit of the supported-host build 15 and its local distribution.
...
class Audit:
    def __init__(self,mode):
        self.failures=[]
        self.data={'verifiedAt':dt.datetime.now(dt.timezone.utc).isoformat(),'mode':mode,'version':'1.0','build':'15','bundleIdentifier':BUNDLE,'apiBaseURL':API,'associatedDomains':DOMAINS}
...
def verify_cloud(args,a):
    ...
    a.require(len(expected)==46,'Frozen source manifest must contain exactly 46 shipping files')
    a.check(manifest.get('build')=='15' and manifest.get('version')=='1.0','Frozen source manifest version/build differs')
    ...
    a.check(len(versions)==6 and all(v.strip()=='15' for v in versions),'Project target build numbers differ')
    ...
    a.check(env.get('build')=='15' and env.get('version')=='1.0','Cloud record version/build differs')
    ...
    a.check(bool(re.fullmatch(r'23[A-Z][0-9]+[a-z]?',env.get('sdkBuild',''))),'Cloud SDK build identifier is malformed')
    ...
    archive=args.cloud_root/'tsutsuura-15-unsigned.xcarchive'
    ...
    a.check(properties.get('CFBundleVersion')=='15','Unsigned archive metadata build differs')
    a.check(properties.get('CFBundleIdentifier')==BUNDLE,'Unsigned archive bundle ID differs')
    a.check(info.get('CFBundleVersion')=='15' and info.get('CFBundleShortVersionString')=='1.0','Unsigned app version/build differs')
...
def verify_upload(args,a):
    ...
    receipts=[r for r in info.get('Distributions',[]) if str(r.get('uploadedBuildNumber'))=='15' and str(r.get('adamId'))==ADAM]
    a.require(bool(receipts),'Matching Apple build 15 receipt missing')
    ...
    a.data['upload']={'adamId':ADAM,'build':'15','event':{k:(v.isoformat() if isinstance(v,dt.datetime) else v) for k,v in event.items() if k in ('date','state','title','errors','warnings')},'processingReported':'Uploaded package is processing.' in log,'testerAvailabilityVerified':False,'appReviewAcceptanceVerified':False}
...
def main():
    ...
    parser.add_argument('--root',type=Path,default=Path('/tmp/tsutsuura-appstore-build15'))
    parser.add_argument('--manifest',type=Path,default=Path('/tmp/tsutsuura-build15-source-manifest.json'))
    parser.add_argument('--expected-commit',required=True)
    parser.add_argument('--expected-run-id')
    parser.add_argument('--cloud-root',required=True,type=Path,help='Extracted tar root containing evidence/ and tsutsuura-15-unsigned.xcarchive/')
    ...
    parser.add_argument('--signed-archive',type=Path,help='Locally signed archive copy containing upload receipt')
