#!/usr/bin/env python3
"""Read-only audit of the supported-host build 16 and its local distribution.
...
class Audit:
    def __init__(self,mode):
        self.failures=[]
        self.data={'verifiedAt':dt.datetime.now(dt.timezone.utc).isoformat(),'mode':mode,'version':'1.0','build':'16','bundleIdentifier':BUNDLE,'apiBaseURL':API,'associatedDomains':DOMAINS}
...
def verify_cloud(args,a):
    ...
    a.check(manifest.get('build')=='16' and manifest.get('version')=='1.0','Frozen source manifest version/build differs')
    ...
    a.check(len(versions)==6 and all(v.strip()=='16' for v in versions),'Project target build numbers differ')
    ...
    a.check(env.get('build')=='16' and env.get('version')=='1.0','Cloud record version/build differs')
    ...
    archive=args.cloud_root/'tsutsuura-16-unsigned.xcarchive'
    ...
    a.check(properties.get('CFBundleVersion')=='16','Unsigned archive metadata build differs')
    a.check(properties.get('CFBundleIdentifier')==BUNDLE,'Unsigned archive bundle ID differs')
    a.check(info.get('CFBundleVersion')=='16' and info.get('CFBundleShortVersionString')=='1.0','Unsigned app version/build differs')
...
def verify_upload(args,a):
    ...
    receipts=[r for r in info.get('Distributions',[]) if str(r.get('uploadedBuildNumber'))=='16' and str(r.get('adamId'))==ADAM]
    a.require(bool(receipts),'Matching Apple build 16 receipt missing')
    ...
    a.data['upload']={'adamId':ADAM,'build':'16','event':{k:(v.isoformat() if isinstance(v,dt.datetime) else v) for k,v in event.items() if k in ('date','state','title','errors','warnings')},'processingReported':'Uploaded package is processing.' in log,'testerAvailabilityVerified':False,'appReviewAcceptanceVerified':False}
...
def main():
    ...
    parser.add_argument('--root',type=Path,default=Path('/tmp/tsutsuura-appstore-build16'))
    parser.add_argument('--manifest',type=Path,default=Path('/tmp/tsutsuura-build16-source-manifest.json'))
    parser.add_argument('--expected-commit',required=True)
    parser.add_argument('--expected-run-id')
    parser.add_argument('--cloud-root',default=Path('/tmp/tsutsuura-cloud16'),type=Path,help='Extracted tar root containing evidence/ and tsutsuura-16-unsigned.xcarchive/')
    ...
    parser.add_argument('--signed-archive',type=Path,default=Path('/tmp/tsutsuura-TestFlight-16.xcarchive'),help='Locally signed archive copy containing upload receipt')
