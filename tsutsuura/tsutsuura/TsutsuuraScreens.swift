...
struct HomeScreen: View {
    ...
    private var showsQuestionBanner: Bool {
        guard let question = feed.todayQuestion, question.answer == nil, question.hasAnswered != true else { return false }
    ...
    private var canRevealUpcomingQuestion: Bool {
        feed.todayQuestion?.isAvailable == false
            ...
            && feed.todayQuestion?.hasAnswered != true
            && !showsQuestionBanner
    }
                    ...
                    if visibleAnswers.isEmpty && !(tab == .family ? isLoadingFamily : isLoadingHistory) {
                        ...
                        ForEach(
                            ...
                            AnswerCard(
                                ...
                                loadMedia: loadMedia,
                                onSafety: answer.author.id == currentUserID ? nil : { onSafety?(answer) }
                            )
        ...
        VStack(spacing: 10) {
            ...
            if let onSafety {
                Button(action: onSafety) {
                    Label("報告・ブロック", systemImage: "hand.raised.fill")
                        .font(TsutsuuraTheme.bodyFont(size: 19))
                        .frame(minHeight: 48)
                }
                .buttonStyle(.plain).foregroundStyle(.white)
                .accessibilityLabel("\(answer.author.displayName)さんの回答を報告・ブロック")
                .accessibilityIdentifier("answer-safety-\(answer.id)")
            }
    ...
    var body: some View {
        ScrollViewReader { scrollProxy in
            LifecyclePage(title: "本日の質問", onBack: goBack) {
                ...
                if question.hasAnswered == true || question.answer != nil || question.answerHidden == true {
                    Text(question.answerHidden == true
                        ? "本日の回答は送信済みです。現在、この回答は表示できません。"
                        : "本日の回答は送信済みです。次の質問をお楽しみに。")
                        .font(TsutsuuraTheme.bodyFont(size: 23))
                        .foregroundStyle(.white)
                        .accessibilityIdentifier("question-already-submitted")
                } else {
                PaperPanel {
                    ZStack(alignment: .topLeading) {
            ...
            VStack(spacing: 0) {
                ...
                ScrollViewReader { scrollProxy in
                    ScrollView(showsIndicators: false) {
                        VStack(spacing: 17) {
                            AnswerCard(
                                ...
                                onComments: {},
                                loadMedia: loadMedia,
                                onSafety: answer.author.id == currentUserID ? nil : onAnswerSafety
                            )
                    ...
                    .onChange(of: comments.map(\.id)) { _, ids in
                        if let reply = replyingTo, !ids.contains(reply.id) { replyingTo = nil }
                    }
    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            ScrollView(showsIndicators: false) {
                ...
                if let onBlockedUsers {
                    LifecycleNavigationButton(title: "ブロックした人", subtitle: "投稿を非表示にした人の確認・解除", icon: "hand.raised.fill", action: onBlockedUsers)
                        .accessibilityIdentifier("blocked-users-button")
                }
