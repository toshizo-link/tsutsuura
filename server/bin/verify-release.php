...
try {
    ...
    if ($status !== 200 || ($response['data']['apiRevision'] ?? null) !== '2026-09-07-safety') {
        throw new RuntimeException('The expected 2026-09-07-safety API release is not healthy or not deployed.');
    }
    $health = $response['data'];
    if (($health['schemaVersion'] ?? null) !== '011_content_safety.sql') {
        throw new RuntimeException('Safety schema011 is not ready.');
    }
    foreach (['familySetup', 'answerMedia', 'accountRecovery', 'accountExport', 'notificationPreferences', 'scheduledQuestions', 'immutableAnswers', 'profileMarks', 'emailAuthentication', 'userBlocking', 'answerReporting', 'commentReporting', 'reportedContentHiding', 'operatorModeration'] as $capability) {
    ...
    foreach ([
        '/v1/me/blocks' => 401,
        '/v1/answers/1/reports' => 405,
        '/v1/comments/1/reports' => 405,
        '/v1/me/export' => 401,
    ...
    foreach (['PUT', 'DELETE'] as $method) {
        [$status, $response] = probe($baseURL . '/v1/me/blocks/1', 'POST', $method);
        if ($status !== 401 || ($response['error']['code'] ?? null) !== 'authentication_required') {
            throw new RuntimeException('Blocking route is unavailable through POST method override.');
        }
    }
    foreach (['/v1/answers/1/reports', '/v1/comments/1/reports'] as $path) {
        [$status, $response] = probe($baseURL . $path, 'POST');
        if ($status !== 401 || ($response['error']['code'] ?? null) !== 'authentication_required') {
            throw new RuntimeException('Reporting route is unavailable.');
        }
    }
