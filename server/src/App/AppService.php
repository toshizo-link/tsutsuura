...
final class AppService
{
    ...
    public function home(int $userId, mixed $cursorValue = null): array
    {
        ...
        $question = $this->todayQuestionRow();
        ...
        return [
            ...
            'todayQuestion' => self::question($question, $today),
    ...
    public function todayQuestion(int $userId): array
    {
        ...
        return [
            'question' => self::question($question, $today),
    ...
    public function updateAnswer(int $userId, string $answerIdValue, mixed $bodyValue): array
    {
        ...
        $body = self::body($bodyValue, 2000, 'answer', true);
        $this->database->transaction(function (PDO $pdo) use ($userId, $answerId, $body): void {
            $answer = $this->ownedAnswer($pdo, $answerId, $userId, true);
            if ($body === '' && (int) $answer['media_count'] === 0) {
                throw new ApiException(422, 'invalid_answer', 'An answer must include text or media.');
            }
            $update = $pdo->prepare(
                'UPDATE answers SET body = :body, updated_at = CURRENT_TIMESTAMP WHERE id = :answer'
            );
            $update->execute(['body' => $body, 'answer' => $answerId]);
        });
        return $this->answer($answerId, $userId);
    }

    public function deleteAnswer(int $userId, string $answerIdValue): void
    {
        $answerId = self::id($answerIdValue, 'answer');
        $storageKeys = $this->database->transaction(function (PDO $pdo) use (
            $userId,
            $answerId,
        ): array {
            $this->ownedAnswer($pdo, $answerId, $userId, true);
            $media = $pdo->prepare('SELECT storage_key FROM answer_media WHERE answer_id = :answer');
            $media->execute(['answer' => $answerId]);
            $storageKeys = array_values(array_map('strval', $media->fetchAll(PDO::FETCH_COLUMN)));
            $delete = $pdo->prepare('DELETE FROM answers WHERE id = :answer AND user_id = :user');
            $delete->execute(['answer' => $answerId, 'user' => $userId]);
            if ($delete->rowCount() !== 1) {
                throw new ApiException(404, 'answer_not_found', 'Answer not found.');
            }
            return $storageKeys;
        });
        $this->answerMedia->deleteStorageKeys($storageKeys);
    }
    ...
    public function deleteAnswerMedia(
        ...
        $answerId = self::id($answerIdValue, 'answer');
        $mediaId = self::id($mediaIdValue, 'media');
        $storageKey = $this->database->transaction(function (PDO $pdo) use (
            $userId,
            $answerId,
            $mediaId,
        ): string {
            $answer = $this->ownedAnswer($pdo, $answerId, $userId, true);
            $media = $pdo->prepare(
                'SELECT storage_key FROM answer_media WHERE id = :media AND answer_id = :answer'
            );
            $media->execute(['media' => $mediaId, 'answer' => $answerId]);
            $storageKey = $media->fetchColumn();
            if ($storageKey === false) {
                throw new ApiException(404, 'media_not_found', 'Answer media not found.');
            }
            if (trim((string) $answer['body']) === '' && (int) $answer['media_count'] <= 1) {
                throw new ApiException(
                    409,
                    'answer_would_be_empty',
                    'Add text before deleting the answer’s last media item.',
                );
            }
            $delete = $pdo->prepare(
                'DELETE FROM answer_media WHERE id = :media AND answer_id = :answer'
            );
            $delete->execute(['media' => $mediaId, 'answer' => $answerId]);
            if ($delete->rowCount() !== 1) {
                throw new ApiException(404, 'media_not_found', 'Answer media not found.');
            }
            return (string) $storageKey;
        });
        $this->answerMedia->deleteStorageKeys([$storageKey]);
        return $this->answer($answerId, $userId);
    }
    ...
    public function comments(int $userId, string $answerIdValue, mixed $cursorValue, mixed $limitValue): array
    {
        ...
        $statement = $this->database->connection()->prepare(
            'SELECT c.id, c.answer_id, c.user_id, c.parent_comment_id,
                    ...
                    u.phone_e164, u.display_name, u.avatar_url, u.created_at AS user_created_at
             FROM comments c
    ...
    /** @return array<string, mixed> */
    private function todayQuestionRow(): array
    {
        $today = $this->todayDate();
        $specific = $this->database->connection()->prepare(
            'SELECT id, prompt FROM questions WHERE available_on = :date AND is_active = 1 LIMIT 1'
        );
        $specific->execute(['date' => $today]);
        $row = $specific->fetch();
        if (is_array($row)) {
            return $row;
        }
        ...
        if ($count === 0) {
            throw new ApiException(503, 'question_unavailable', 'Today’s question is not available.');
        }
        $offset = (int) (sprintf('%u', crc32($today)) % $count);
        $statement = $this->database->connection()->prepare(
            'SELECT id, prompt FROM questions
             WHERE available_on IS NULL AND is_active = 1
     ...
     *  @return array<string, mixed>
     */
    private static function question(array $row, string $date): array
    {
        return ['id' => (string) $row['id'], 'prompt' => (string) $row['prompt'], 'date' => $date];
    }
